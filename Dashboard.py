...
with st.sidebar.expander('Setor'):
  ...
  todos_setores = st.checkbox('Todos os setores', value=True)

  if todos_setores:
    f_setores = (setores)
  else:
    setores = dados['Setor de emissão'].unique()
    f_setores = st.multiselect('Selecione os setores', setores)

with st.sidebar.expander('Gás'):
  gases = dados['Gás'].unique()
  todos_gases = st.checkbox('Todos os gases', value=True)
...
with st.sidebar.expander('Estados ou regiões'):
  ...
  if filtro_regiao:
    ...
    if regiao == 'Brasil':
      ...
      f_estados = ['SP', 'RJ', 'MG', 'ES']
    elif regiao == 'Sul':
      f_estados = ['RS', 'SC', 'PR']
  else:
    f_estados = st.multiselect('Selecione os estados', estados)
...
## Estados
emissoes_estados = dados.groupby('Estado')[['Emissão']].sum().sort_values(by='Emissão', ascending=False).reset_index()
...
## Estados
fig_emissoes_estado_gas = px.sunburst(
  emissoes_estado_gas,
  path=['Estado', 'Gás'],
  values='Emissão',
  color='Gás',
...
fig_emissoes_estado_setor = px.bar(
  emissoes_estado_setor,
  y='Emissão',
  x='Estado',
...
fig_mapa_emissoes = px.scatter_geo(
  emissoes_estados,
  lat='lat',
  lon='long',
  scope='south america',
